...
class DataPrepare(object):
    def __init__(self, dataset,word_embedding_path=None,data_path=None):
        self.tag2Idx = {
        ...
        self.words = self.get_words(dataset,data_path)
        self.word2Idx = {}  # word
        self.wordEmbeddings = []  
        with open(word_embedding_path+"/glove.6B.100d.txt", "r", encoding='utf-8') as fw:
            for line in fw:
                line = line.strip()
                splits = line.split(" ")
        ...
        with open(filename, "r", encoding='utf-8') as fw:
            ...
            for line in fw:
                if len(line) == 0 or line.startswith('-DOCSTART') or line[0] == '\n':
                    ...
                    # if len(splits) != 3:
                    #     continue
                    # print(filename, line, splits)
                    if len(splits[0].strip()) > 0:
                        if splits[1].strip() not in ["-1", "0"] and len(splits) == 3:
                            sentence.append([splits[0].strip(), int(
                                splits[1].strip() == "B-" + flag or splits[1].strip() == "I-" + flag), int(splits[2])])
                        else:
                            sentence.append([splits[0].strip(), -1, int(splits[-1])])
    ...
    def get_words(self, dataset,data_path):
        words = {}
        if dataset == "conll2003" or dataset == 'conll2002' or dataset == 'mydata':

            trainSentences = self.read_origin_file(data_path+"/" + dataset + "/train.txt")
            validSentences = self.read_origin_file(data_path+"/" + dataset + "/valid.txt")
            testSentences = self.read_origin_file(data_path+"/" + dataset + "/test.txt")
