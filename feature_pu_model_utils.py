...
class FeaturedDetectionModelUtils(ModelUtils):
    def __init__(self, dp,dictionary_path=None):
    ...
    def add_dict_info(self, sentences, windowSize, datasetName):
        ...
        toxiBigDic = set()
        # miscBigDic = set()
        with open(self.dictionary_path + datasetName + "/cancerBigDic.txt", "r",encoding='utf-8') as fw:
            for line in fw:
                line = line.strip()
                if len(line) > 0:
                    cancerBigDic.add(line)
        with open(self.dictionary_path + datasetName + "/drugBigDic.txt", "r",encoding='utf-8') as fw:
            for line in fw:
                line = line.strip()
                if len(line) > 0:
                    drugBigDic.add(line)
        with open(self.dictionary_path + datasetName + "/toxiBigDic.txt", "r",encoding='utf-8') as fw:
            for line in fw:
                line = line.strip()
                if len(line) > 0:
                    toxiBigDic.add(line)
...
#                         miscBigDic.add(line)
        for i, sentence in enumerate(sentences):
            for j, data in enumerate(sentence):
                ...
                for idx, word in enumerate(words):
                    ...
                    if word in cancerBigDic:
                        feature[self.dp.tag2Idx["CANCER"]][count - 1] = 1
                    elif word in drugBigDic:
                        feature[self.dp.tag2Idx["DRUG"]][count - 1] = 1
                    elif word in toxiBigDic:
                        feature[self.dp.tag2Idx["TOXI"]][count - 1] = 1
    ...
    def load_dataset(self, flag, datasetName, percent,data_path,enhanced=False):
        fname = data_path + datasetName + "/train." + flag + ".txt"
        ...
        validSentences = self.dp.read_processed_file(data_path + datasetName + "/valid.txt", flag)
        ...
        testSentences = self.dp.read_processed_file(data_path + datasetName + "/test.txt", flag)
        ...
        if enhanced:
          trainSet = trainSet + validSet+testSet#[0:len(testSet)//2]
